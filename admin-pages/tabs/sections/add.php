 ...
 */
if( !class_exists('NHS_SectionsAddTabAdminPage') ):
class NHS_SectionsAddTabAdminPage extends APL_TabAdminPage
{
	/**
	...
	public function process_settings( $settings, $option )
	{
		...
		if( $this->model->get_section_by_name($settings['name']) !== NULL || $this->model->get_section_by_key($key) !== NULL )
		{
		...
		// set new section.
		$this->model->add_section( $key, $settings );
		return $this->model->get_sections();
	}
	...
	public function display()
	{
		...
		$this->section = $this->model->get_default_section();
